...
P2P.NO_LISTEN = false;
P2P.MAX_BLOCKS = 500;
P2P.STALE_TIP_AGE = 20 * 60 * 1000;
P2P.SYNC_TIMEOUT = 30000;
...
P2P.prototype.initialize = function() {
  ...
  this.chain.on('queueprocessed', this._onChainQueueProcessed.bind(this));
...
P2P.prototype._onPeerInv = function(peer, message) {
  ...
  log.debug('Received inv from ' + peer.host);
  ...
  var filtered = inventory.filter(function(a) {
    ...
    } else if (a.type === Inventory.TYPE.BLOCK && !self.disableSync) {
  ...
  if (filtered.length) {
    ...
    log.debug('Sent getdata message');
...
P2P.prototype._onPeerGetData = function(peer, message) {
  ...
  log.debug('Received getdata from ' + peer.host);
  var inventory = message.inventory;
...
P2P.prototype._onChainQueueProcessed = function() {
  if(!this.synced) {
    this._sync();
  }
};
...
P2P.prototype._sync = function(peer) {
  ...
  if((self.chain.tip.timestamp.getTime() > (Date.now() - self.staleTipAge)) && !self.synced) {
    ...
    // Emit synced if the tip does not change after 30 seconds
    setTimeout(function() {
      if(self.chain.tip.hash === tipHash && !self.synced) {
        self.synced = true;
        self.emit('synced');
      }
    }, self.syncTimeout);
  ...
  self.chain.getHashes(self.chain.tip.hash, function(err, hashes) {
    ...
    if(peer) {
      ...
      log.debug('Sent getblocks message');
    }
  });
};
...
P2P.prototype._onPeerGetBlocks = function(peer, message, callback) {
  ...
  log.debug('Received getblocks from ' + peer.host);
