...
DB.prototype.getWeight = function(blockHash, callback) {
  var key = [this.weightPrefix, blockHash].join('-');
  this.store.get(key, function(err, weightData) {
    if(err) {
      return callback(err);
    }

    callback(null, new BN(weightData, 'hex'));
  });
};
...
DB.prototype._updateWeight = function(blockHash, weight, callback) {
  this.store.put([this.weightPrefix, blockHash].join('-'), weight.toString(16), callback);
};
